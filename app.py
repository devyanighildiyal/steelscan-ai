...
Streamlit Prototype · Devyani Ghildiyal | Manipal University Jaipur | 229301091

Architecture exactly matches DualPathAFNet_v2_best.pth checkpoint keys.
"""
...
[data-testid="stSidebar"] { background-color: #090b0f !important; border-right: 1px solid #1e2330; }
...
.stButton > button {
    background: #3d7aed !important; color: white !important; border: none !important;
...
.app-subtitle { font-size: 13px; color: #5a6478; line-height: 1.7; max-width: 660px; margin-bottom: 28px; font-style: italic; }
.section-label { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: #3d4658; letter-spacing: 0.14em; text-transform: uppercase; margin-bottom: 10px; }
.card { background: #13161f; border: 1px solid #1e2330; border-radius: 12px; padding: 20px 22px; margin-bottom: 14px; }
.result-class { font-family: 'Inter', sans-serif; font-size: 30px; font-weight: 700; letter-spacing: -0.02em; text-transform: capitalize; margin-bottom: 4px; }
.result-conf { font-size: 13px; color: #22c55e; margin-bottom: 14px; font-weight: 500; }
.mono { font-family: 'JetBrains Mono', monospace; font-size: 11px; }
.tag { display: inline-block; font-family: 'JetBrains Mono', monospace; font-size: 10px; font-weight: 500; padding: 3px 8px; border-radius: 4px; letter-spacing: 0.04em; }
...
.info-box { background: rgba(61,122,237,0.07); border: 1px solid rgba(61,122,237,0.2); border-radius: 8px; padding: 12px 14px; font-size: 13px; color: #8892a4; line-height: 1.6; }
.warn-box { background: rgba(245,158,11,0.07); border: 1px solid rgba(245,158,11,0.2); border-radius: 8px; padding: 12px 14px; font-size: 13px; color: #fbbf24; line-height: 1.6; }
.divider { border: none; border-top: 1px solid #1e2330; margin: 18px 0; }
.arch-step { display: flex; align-items: flex-start; gap: 10px; padding: 9px 10px; border-radius: 7px; border: 1px solid #1e2330; margin-bottom: 6px; background: #0d1017; }
...
.empty-state { background: #13161f; border: 1px solid #1e2330; border-radius: 12px; padding: 60px 24px; text-align: center; }
</style>
""", unsafe_allow_html=True)


# ─────────────────────────────────────────────────────────────────────────────
# MODEL — exactly matching checkpoint keys from debug output
#
# Key observations from checkpoint:
#   mscb.fc.0.weight  → shape [256, 4080]  so fused_dim = 4080
#   afg.gate_mlp.0    → Linear (not Sequential index 0 of outer Sequential)
#   afg.gate_mlp.3    → the final Linear before Sigmoid  (indices 0,1,2,3,4)
#   classifier.0      → LayerNorm  (key: classifier, NOT head)
#   classifier.2      → Linear 384→256
#   classifier.5      → Linear 256→6
# ─────────────────────────────────────────────────────────────────────────────
...
class ChannelPyramidPooling(nn.Module):
    def __init__(self, in_channels_list, out_dim=256):
        ...
        # 3 scales × (out_dim//3) channels × 4×4 spatial = 3 × 85 × 16 = 4080
        fused_dim = (out_dim // 3) * 3 * 4 * 4   # = 4080 ✅ matches checkpoint
...
class GlobalContextTransformerBranch(nn.Module):
    def __init__(self, in_channels=320, d_model=128, nhead=4, num_layers=2, dropout=0.1):
        ...
        self.input_proj = nn.Sequential(           # keys: gctb.input_proj.0 / .1
        ...
        encoder_layer   = nn.TransformerEncoderLayer(
            ...
            dim_feedforward=d_model * 4,           # 512 ✅ matches checkpoint linear1/linear2
...
class AdaptiveFusionGate(nn.Module):
    def __init__(self, cnn_dim, trans_dim, shared_dim=256):
        super().__init__()
        self.cnn_proj   = nn.Linear(cnn_dim, shared_dim)      # afg.cnn_proj  ✅
        self.trans_proj = nn.Linear(trans_dim, shared_dim)    # afg.trans_proj ✅
        combined_dim    = cnn_dim + trans_dim                  # 384
        # gate_mlp keys: .0 (Linear 384→96), .1 (GELU), .2 (Dropout), .3 (Linear 96→1), .4 (Sigmoid)
        self.gate_mlp   = nn.Sequential(
            nn.Linear(combined_dim, combined_dim // 4),  # [0] 384→96 ✅
            nn.GELU(),                                   # [1]
            nn.Dropout(0.1),                             # [2]
            nn.Linear(combined_dim // 4, 1),             # [3] 96→1 ✅
            nn.Sigmoid()                                 # [4]
        )
...
class DualPathAFNet(nn.Module):
    def __init__(self, num_classes=6, cnn_out_dim=256,
                 trans_d_model=128, shared_dim=256,
                 num_layers=2, dropout=0.3):
        super().__init__()
        self.stem = timm.create_model('mobilenetv2_100', pretrained=False, features_only=True)
        self.mscb = ChannelPyramidPooling(
            in_channels_list=[24, 96, 320],
            out_dim=cnn_out_dim
        )
        self.gctb = GlobalContextTransformerBranch(
            in_channels=320, d_model=trans_d_model,
            nhead=4, num_layers=num_layers, dropout=0.1
        )
        ...
        # classifier keys: .0 LayerNorm, .2 Linear 384→256, .5 Linear 256→6  ✅
        classifier_in = shared_dim + trans_d_model   # 256 + 128 = 384
        self.classifier = nn.Sequential(
            nn.LayerNorm(classifier_in),             # [0] ✅
            nn.Dropout(dropout),                     # [1]
            nn.Linear(classifier_in, 256),           # [2] ✅
            nn.GELU(),                               # [3]
            nn.Dropout(dropout / 2),                 # [4]
            nn.Linear(256, num_classes)              # [5] ✅
        )

    def forward(self, x, return_alpha=False):
        feats      = self.stem(x)
        s1         = feats[1]    # 24ch  56×56
        s2         = feats[3]    # 96ch  14×14
        s3         = feats[4]    # 320ch  7×7
...
KNOWN_ALPHA = {
    'crazing': 0.971, 'inclusion': 0.343, 'patches': 0.975,
...
def predict(model, device, image):
    ...
    return CLASS_NAMES[pred_idx], float(probs[pred_idx]), probs, alpha_val, latency
...
def conf_chart(probs):
    ...
    fig.patch.set_facecolor(BG_CHART); ax.set_facecolor(BG_CHART)
    ...
    ax.set_xlim(0, 1.08)
    ax.set_xlabel('Confidence', color='#5a6478', fontsize=9)
    ...
    for bar, v in zip(ax.patches, vals):
        ax.text(v + 0.02, bar.get_y() + bar.get_height() / 2,
                f'{v:.1%}', va='center', color='#5a6478', fontsize=9)
    ax.grid(axis='x', color='#1e2330', lw=0.7, zorder=1)
    plt.tight_layout()
    buf = io.BytesIO()
    plt.savefig(buf, format='png', dpi=130, bbox_inches='tight', facecolor=BG_CHART)
...
def alpha_chart(pred_class, alpha_val):
    ...
    ax.bar(range(len(classes)), alphas, color=cols, edgecolor=edges,
           linewidth=lws, zorder=3, width=0.55)
    ax.axhline(0.5, color='#2a3040', ls='--', lw=1.2, zorder=2)
    ax.axhline(alpha_val, color='#fbbf24', ls='-', lw=1.5, zorder=4, alpha=0.8)
    ax.set_xticks(range(len(classes)))
    ax.set_xticklabels(
        [c.replace('_', '\n').replace('-in', '\n-in') for c in classes],
        fontsize=7.5, color='#5a6478'
    )
    ax.set_ylim(0, 1.08)
    ax.set_ylabel('α value', color='#5a6478', fontsize=9)
    ax.tick_params(colors='#5a6478', labelsize=9)
    for spine in ax.spines.values(): spine.set_edgecolor('#1e2330')
    ...
    ax.legend(handles=[p1, p2, p3], fontsize=7.5, facecolor=BG_CHART,
              edgecolor='#1e2330', labelcolor='#8892a4', loc='upper right')
    plt.tight_layout()
    buf = io.BytesIO()
    plt.savefig(buf, format='png', dpi=130, bbox_inches='tight', facecolor=BG_CHART)
    buf.seek(0); plt.close(); return buf
...
def render_sidebar():
    with st.sidebar:
        ...
        <div style="font-size:11px;color:#3d4658;line-height:1.6;margin-bottom:20px;
                    font-style:italic;border-left:2px solid #1e2330;padding-left:10px">
          Deep Learning Models for Real-Time<br>Industrial Surface Defect Detection
        </div>
        ...
        st.markdown('<hr style="border:none;border-top:1px solid #1e2330;margin:16px 0">',
                    unsafe_allow_html=True)
        st.markdown('<div class="section-label">Architecture</div>', unsafe_allow_html=True)
        st.markdown("""
        <div class="arch-step">
          <div class="arch-dot" style="background:#3d7aed"></div>
          <div class="arch-text"><b>CNN Branch</b> — Multi-Scale Pyramid Pooling at 3 scales</div>
        </div>
        <div class="arch-step">
          <div class="arch-dot" style="background:#8b5cf6"></div>
          <div class="arch-text"><b>Transformer Branch</b> — CLS Token + 2-layer Self-Attention</div>
        </div>
        <div class="arch-step">
          <div class="arch-dot" style="background:#22c55e"></div>
          <div class="arch-text"><b>Adaptive Gate (AFG)</b> — Learnable α per sample</div>
        </div>
        <div style="font-size:11px;color:#3d4658;margin-top:8px;line-height:1.7">
          Backbone: MobileNetV2 (pretrained)<br>Params: 3.58M · Transformer layers: 2
        </div>
        """, unsafe_allow_html=True)
...
def main():
        ...
        with c1:
            st.markdown("""<div class="card">
              ...
              <div style="margin-top:10px;font-size:13px;color:#8892a4;line-height:1.7">
                Uses MobileNetV2 features at 3 scales (56×56, 14×14, 7×7) pooled into a
                256-dim local feature vector. Captures textures, edges, and fine-grain patterns.
        ...
        with c2:
            st.markdown("""<div class="card">
              ...
              <div style="margin-top:10px;font-size:13px;color:#8892a4;line-height:1.7">
                ...
                2 self-attention layers to capture long-range spatial relationships.
              </div></div>""", unsafe_allow_html=True)
        with c3:
            st.markdown("""<div class="card">
    ...
    if loaded:
        st.markdown(f"""<div class="info-box">
          ✅ <strong>DualPath-AFNet v2 loaded successfully</strong> · 3.58M parameters ·
          Running on <strong>{dlabel}</strong>{dnote}
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""<div class="warn-box">
          ⚠️ <strong>Demo mode</strong> — <code>DualPathAFNet_v2_best.pth</code> not found.
          Place it in the same folder as app.py. Predictions will not be meaningful.
          Running on {dlabel}.
        </div>""", unsafe_allow_html=True)
    ...
    with col_l:
        ...
        uploaded = st.file_uploader(
            "Upload", type=["png", "jpg", "jpeg", "bmp", "tiff", "tif"],
    ...
    with col_r:
        ...
        if uploaded and run:
            ...
            insight = (
                'Using <strong style="color:#5b9cf6">local texture features</strong> '
                'from the CNN branch — typical for texture-based defects.'
                if av > 0.5 else
                'Using <strong style="color:#a78bfa">global spatial context</strong> '
                'from the Transformer — the defect requires whole-image reasoning.'
            )

            st.markdown(f"""<div class="card">
              <div class="section-label">Predicted class</div>
              <div class="result-class" style="color:{color}">{pred.replace('_',' ')}</div>
              <div class="result-conf">Confidence {conf:.1%}</div>
              <div style="background:#0d1017;border-radius:5px;height:5px;
            ...
            st.markdown("""<div class="empty-state" style="min-height:400px">
              ...
              <div style="font-size:12px;color:#2a3040;margin-top:4px">
